...
import { initialRoom, changeRoom, actions, normalizeRoom } from '@/lib/room';
...
export async function GET(req:Request){try{const id=key(req);const row=await roomDb().prepare('SELECT state FROM rooms WHERE id = ?').bind(id).first<{state:string}>();return json({state:row?normalizeRoom(JSON.parse(row.state)):initialRoom});}catch(e){return json({error:(e as Error).message},400);}}
export async function POST(req:Request){
 ...
 const state=changeRoom(normalizeRoom(JSON.parse(row.state)),data.action,data.payload,data.person);
